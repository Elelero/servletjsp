...
public class ContentController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		board.setBno(1);
		board.setBtitle("금요일");
		board.setBcontent("열심히 공부합니다...^^ㅗ");
		
		//request 저장 범위 (요청에서 응답까지만 공유)
		request.setAttribute("user", user); //User DTO 넣기
		
		//session 저장 범위 (같은 클라이언트에서 공유)
		HttpSession session = request.getSession();
		session.setAttribute("board", board); //Board DTO 넣기
		
		//servletContext(application) 저장 범위 (모든 클라이언트에서 공유)
